...
import { GET_LEAVE_INTIMATIONS } from "./queries";

export const getLeaveIntimations = async (status?: string) => {
  try {
    ...
    return {
      payload: formatLeaveIntimations(data?.leaveIntimations ?? []),
    };
  } catch (err: any) {
    const msg =
...
const formatLeaveIntimations = (items: any[]) => {
  const months = [
    "January",
    "February",
    "March",
